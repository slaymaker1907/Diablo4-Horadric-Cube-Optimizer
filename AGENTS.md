...
- Every run carries a hard step budget (`payload.maxSteps`, default 200 via `normalizeMaxStepsV3` — there is no unlimited mode). Exceeding it is failure, like a GA break. The fast decomposition layer is gated to `expectedSteps <= maxSteps/4`; other cases route to the finite-horizon budget DP (`v3-residual-budget-dp`). Residual LAO* and refinement are exported for tests but unrouted.
