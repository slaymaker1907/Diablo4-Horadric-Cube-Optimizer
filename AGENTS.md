...
- Gear-slot legality comes from `gear-slot-legality.js`. Concrete slots must narrow the legal affix pool; `Any` must preserve the unrestricted pool. To refresh the slot → affix table from Diablo4Companion data run `node scripts/build-slot-legality.js` (downloads fresh JSON to `scripts/data/`, which is gitignored; review the printed diff before editing `gear-slot-legality.js`).
